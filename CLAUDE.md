...
**Date convention:** every file in `content/locations/` and `content/blog/` must have a `date: "YYYY-MM-DD"` in its frontmatter set to the real last edit. Without it the sitemap's `<lastmod>` falls back to file mtime, which becomes the deploy date on Amplify's fresh clone. `pnpm run generate-sitemap` warns and names any file that is missing it.
